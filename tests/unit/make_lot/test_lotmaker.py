...
from opv_import.makelot import LotMaker, CameraImage, ImageSet, CameraImageFetcher
from opv_import.makelot.lotMaker import SearchedRefImgSet
...
class TestLotMaker(object):
    ...
    def test_cam_lot_generator(self, fetcher_test_env):
        lm = LotMaker(pictures_path=Path("picPath/"), rederbro_csv_path=None, nb_cams=2)
        lm.load_cam_images()
        ...
        set_gen = lm.cam_lot_generator(reference_set=reference_set)

        assert set_gen.__next__() == sets[0]
        assert set_gen.__next__() == sets[1]
        assert set_gen.__next__() == sets[2]
        assert set_gen.__next__() == sets[3]
