...
class LotMaker:
    ...
    def make_gopro_set_new(
            ...
            threshold_incomplete_set_max_in_window) -> List[ImageSet]:
        """
        """
        cam_max_indexes = [f.nb_pic() - 1 for f in self.fetchers]
        current_indexes = [0] * self.nb_cams
        set_partitions = []

        # partition while indexes aren't all consumed
        while current_indexes < cam_max_indexes:
            partition_start = current_indexes

            p = self.generate_cam_partition(
                partition_start,
                threshold_max_consecutive_incomplete_sets,
                threshold_incomplete_set_window_size,
                threshold_incomplete_set_max_in_window)
            set_partitions.append(p)
            current_indexes = p.fetcher_next_indexes

        # compile partitions
        gp_sets = []
        # compile partitions into gp_sets TODO
        return gp_sets
    ...
    def generate_cam_partition(
            ...
            threshold_max_consecutive_incomplete_sets,
            threshold_incomplete_set_window_size,
            threshold_incomplete_set_max_in_window) -> CameraSetPartition:
        """
        """
        ...
        # TODO handle window incomplete set count
        ...
        for cam_indexes in indexe_gen:
            ...
            img_set_generator = self.cam_set_generator(reference_set=cam_set, start_indexes=list(fetcher_next_indexes))
            ...
            try:
                ...
                if not reject:
                    fetcher_next_indexes = gen_img_set_with_fetcher_indexes.fetcher_next_indexes
